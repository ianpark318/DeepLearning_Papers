...
class VGGBase(nn.Module):
    ...
    def load_pretrained_layers(self):
        """
        As in the paper, we use a VGG16 pretrained on the ImageNet task as the base network.
        There's one available in PyTorch, see https://pytorch.org/docs/stable/torchvision/models.html#torchvision.models.vgg16
        We copy these parameters into our network.  It's straightforward for conv1 to conv5.
        However, the original VGG16 does not contain the conv6 and conv7 layers.
...
class PredictionConvolutions(nn.Module):
    ...
    The class scores rerpresent the scores of each object class in each of the 8732 bounding boxes located.
...
class SSD300(nn.Module):
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        For each class, perform Nom-Maximum Suppression (NMS) on boxes that are above a minimum threshold.

        :param predicted_locs: predicted locations/boxes w.r.t the 8732 prior boxes, a tensor of dimensions (N, 8732, 4)
        :param predicted_scores: class scores for each of the encoded locations/boxes, a tensor of dimensions (N, 8732, n_classes)
        :param min_score: minimum threshold or a box to be considered a match for a certain class
        :param max_overlap: maximum overlap two boxes can have so that the one with the lower scorer is not suppressed via NMS
        :param top_k: if there are a lot of resultijng detection across all classes, keep only the top 'k'
